...
export default function (width, height, ctx) {
  var GAME = {
    keys: [
      {
        ...
        key: 16,
        fnDown: () => {
          GAME.navigator.player.shoot = true;
        },
        fnUp: () => {
          GAME.navigator.player.shoot = false;
        }
      }, {
        key: 32,
        fnDown: () => {
          GAME.navigator.player2.shoot = true;
        },
        fnUp: () => {
          GAME.navigator.player2.shoot = false;
        }
      }
    ],
    width,
    ...
    navigator: {
      'lastUpdate': Date.now(),
      'player': { forward: false, backward: false, up: false },
      'player2': { forward: false, backward: false, up: false }
    },
  ...
  GAME.whatIsOn = function (x, y, doNormalization, player) {
    ...
    for (let npc of GAME.npcs.filter(npc => {
      return player ? true : !npc.dead;
    })) {
